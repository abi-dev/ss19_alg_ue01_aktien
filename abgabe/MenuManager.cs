...
namespace ue01_aktien
{
    public class MenuManager
    {
        ...
        private readonly Hashtable<Share> _abbrHashtable =
            new Hashtable<Share>(Tablesize, (key, share) => key == share.Abbr);

        // two hashtable store references to shares, using the share name and abbr as keys, constructor gets compare func
        private readonly Hashtable<Share> _nameHashtable =
            new Hashtable<Share>(Tablesize, (key, share) => key == share.Name);
        ...
        private void HandleAdd()
        {
            ...
            try
            {
                ...
                if (_cmd.Args.Length != 3)
                {
                    msg = "Ungültige Anzahl an Parametern.";
                    throw new FormatException(msg);
                }

                var shareToInsert = new Share();
                ...
                var searchRes = _abbrHashtable.Search(_cmd.Args[2]) ?? _nameHashtable.Search(_cmd.Args[0]);
        ...
        private void HandleSearch()
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (searchRes != null) Console.WriteLine("Aktie durch den Namen {0} gefunden!", searchRes.Name);
                }
        ...
        private void HandleDelete()
        {
            ...
            try
            {
                ...
                var res = _abbrHashtable.Search(_cmd.Args[0]) ?? _nameHashtable.Search(_cmd.Args[0]);
        ...
        private void HandleSave()
        {
            ...
            try
            {
                ...
                Stream stream = new FileStream(Directory.GetCurrentDirectory() + "/saves/" + _cmd.Args[0] + ".bin",
                    FileMode.Create, FileAccess.Write,
                    FileShare.None);
                formatter.Serialize(stream, _nameHashtable);
                stream.Close();

                Console.WriteLine("Die Daten wurden unter \"{0}\" gespeichert.", _cmd.Args[0]);
                Console.WriteLine("Zurück zum Menü mit irgendeiner Taste...");
                Console.ReadKey();
                ShowMenu();
            }
            catch (Exception ex)
            {
                if (ex is FormatException)
                    Console.WriteLine(ex.Message);
                else if (ex is FileNotFoundException) Console.WriteLine("Datei nicht gefunden.");
        ...
        private void HandleLoad()
        {
            ...
            try
            {
                ...
                for (var i = 0; i < Tablesize; i++)
                    ...
                    {
                        ref var shareToInsert = ref readHashtable.Items[i];
                        _abbrHashtable.Add(ref shareToInsert, shareToInsert.Abbr);
                    }

                Console.WriteLine("Die Daten wurden aus \"{0}\" geladen.", _cmd.Args[0]);
                Console.WriteLine("Zurück zum Menü mit irgendeiner Taste...");
                Console.ReadKey();
                ShowMenu();
            }
        ...
        private void HandleImport()
        {
            ...
            try
            {
                ...
                var searchRes = _abbrHashtable.Search(_cmd.Args[0]) ?? _nameHashtable.Search(_cmd.Args[0]);
                ...
                if (searchRes != null) // found a share for the file name
                {
                    ...
                    var csvParser = new TextFieldParser(
                        Directory.GetCurrentDirectory() + "/imports/" + _cmd.Args[0] + ".csv");
                    ...
                    while (!csvParser.EndOfData)
                    {
                        ...
                        var date = DateTime.Parse(fields[0]); // parse date column

                        // save data from current line
                        var priceToInsert = new SharePrice(date, float.Parse(fields[1]),
                    ...
                    Console.WriteLine(
                        "Daten für die Aktie \"{0}\" erfolgreich importiert.", searchRes.Name);
                }
                else
                {
                    Console.WriteLine(
                        "Keine Aktie unter diesem Namen gefunden. Vor dem Importieren muss die Aktie anlegt worden sein.");
                }
            ...
            try
            {
                ...
                if (searchRes != null)
                {
                    ...
                    else
                    {
                        ...
                        for (var i = 0; i < searchRes.SharePrices.Length; i++)
                        {
                        ...
                        Console.Write("Datum erster Wert: {0}",
                        ...
                        for (var i = 0; i < searchRes.SharePrices.Length; i++)
                        {
                            ...
                            var value = (int) ((searchRes.SharePrices[i].Close - minValue) / (maxValue - minValue) *
                                               10);
                            var endY = y - value;
        ...
        {
            for (var i = 0; i < Console.BufferWidth; i++) Console.Write("━");
            Console.WriteLine();
        }
